...
public class MagicCameraInputFilter extends MyGPUImageFilter {
    ...
    public void onInit() {
        ...
        setBeautyLevel(BeautyCamera.beautyLevel);
    }
...
//        return mFrameBufferTextures[0];
    ...
    public int onDrawToTexture(final int textureId, int startX, int startY, int viewportW, int viewportH) {
        if (mFrameBuffers == null)
            return OpenGlUtils.NO_TEXTURE;
        runPendingOnDrawTasks();
        GLES20.glViewport(0, 0, mFrameWidth, mFrameHeight);
    ...
    public void initCameraFrameBuffer(int width, int height) {
        if (width == mFrameWidth && height == mFrameHeight && mFrameBuffers != null) {
            return;
        }
    ...
    // TODO: 21/11/2016 这个是干嘛的，目前这个类的美颜关键就是这个 ，暂时改成public
