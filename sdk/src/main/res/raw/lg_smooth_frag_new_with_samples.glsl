...
// 对所有点都 进行采样，但是只对人脸会 实际产生影响，经过测试性能并没有降低
...
void main()
{
    ...
    mediump float tmpCurY = curY;
    tmpCurY /= 255.0;
    
    pixSave += doSome(tmpCurY, mW - mXStep * 10.0, mH - mYStep * 10.0, fenziY1, fenmuY1, fenziY2, fenmuY2);
    pixCountR ++;
    
    pixSave += doSome(tmpCurY, mW - 0.5 * mXStep * 10.0, mH - 0.5 * mYStep * 10.0, fenziY1, fenmuY1, fenziY2, fenmuY2);
    pixCountR ++;
    
    pixSave += doSome(tmpCurY, mW, mH - mYStep * 10.0, fenziY1, fenmuY1, fenziY2, fenmuY2);
    pixCountR ++;
    
    pixSave += doSome(tmpCurY, mW, mH - 0.5 * mYStep * 10.0, fenziY1, fenmuY1, fenziY2, fenmuY2);
    pixCountR ++;
    ...
    pixSave += doSome(tmpCurY, mW - mXStep * 10.0, mH, fenziY1, fenmuY1, fenziY2, fenmuY2);
    pixCountR ++;
    
    pixSave += doSome(tmpCurY, mW - 0.5 * mXStep * 10.0, mH, fenziY1, fenmuY1, fenziY2, fenmuY2);
    pixCountR ++;
    ...
    pixCountR ++;
     ...
     if((pixSave  *  5) > (pixCountR * 4)) {
         if(fenmuY1 > 0.0) {
             ...
             tmpCurY = tmpCurY - (tmpCurY - tempY) * smoothXishu/(100.0/255.0);
         }
     } else {
         if(fenmuY2 > 0.0) {
